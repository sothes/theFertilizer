 ...
 public class Controller extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
	...
	protected void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action == null){
			...
			model.setChangePresentIngredientId(fertiliserId, ingredientId);
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
			...
			model.setChangePresentIngredientId("-1", "-1");
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
...
//			model.setPresentIngredientActive(fertiliserId, ingredientId, false);
			...
			model.setChangeRequiredIngredientId("-1", "-1");
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
			...
			String fertiliserId = request.getParameter("requiredFertiliserId");
			String ingredientId = request.getParameter("requiredIngredientId");
			model.setChangeRequiredIngredientId(fertiliserId, ingredientId);
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
		...
		else if(action.equals("45_editPresentFertiliser")){
			String fertiliserId = request.getParameter("fertiliserId");
		...
		else if(action.equals("46_saveEditPresentFertiliser")){
			String fertiliserName = request.getParameter("fertiliserName");
			String amount = request.getParameter("fertiliserAmount");
			String unit = request.getParameter("fertiliserUnit");
			model.addOrChangePresentFertiliser(fertiliserName, amount, unit);
			model.setChangePresentFertiliser("-1");
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
		else if(action.equals("47_showDeletedPresentFertilisers")){
			model.setShowDeletedPresentFertiliser(true);
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
		else if(action.equals("48_dontshowDeletedPresentFertilisers")){
			model.setShowDeletedPresentFertiliser(false);
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
		else if(action.equals("49_deletePresentFertilisers")){
			String fertiliserId = request.getParameter("fertiliserId");
			model.setPresentFertiliserActive(fertiliserId, false);
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
		else if(action.equals("50_undeletePresentFertilisers")){
			String Str_fertiliserId = request.getParameter("fertiliserId");
			model.setPresentFertiliserActive(Str_fertiliserId, true);
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
		else if(action.equals("51_addRequiredFertiliser")){
			String name = request.getParameter("fertiliserName");
			String amount = request.getParameter("fertiliserAmount");
			String unit = request.getParameter("fertiliserUnit");
			model.addOrChangeRequiredFertiliser(name, amount, unit);
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
		else if(action.equals("52_showRowAddingRequiredFertiliser")){
			model.setAddRequiredFertiliser(true);
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
		else if(action.equals("53_NoShowRowAddingRequiredFertiliser")){
			model.setAddRequiredFertiliser(false);
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
		else if(action.equals("54_editRequiredFertiliser")){
			String fertiliserId = request.getParameter("fertiliserId");
		...
		else if(action.equals("55_saveEditRequiredFertiliser")){
			String fertiliserName = request.getParameter("fertiliserName");
			String amount = request.getParameter("fertiliserAmount");
			String unit = request.getParameter("fertiliserUnit");
			model.addOrChangeRequiredFertiliser(fertiliserName, amount, unit);
			model.setChangeRequiredFertiliser("-1");
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
		else if(action.equals("56_showDeletedRequiredFertilisers")){
			model.setShowDeletedRequiredFertiliser(true);
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
		else if(action.equals("57_dontshowDeletedRequiredFertilisers")){
			model.setShowDeletedRequiredFertiliser(false);
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
		else if(action.equals("58_deleteRequiredFertilisers")){
			String fertiliserId = request.getParameter("fertiliserId");
			model.setRequiredFertiliserActive(fertiliserId, false);
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
		else if(action.equals("59_undeleteRequiredFertilisers")){
			String Str_fertiliserId = request.getParameter("fertiliserId");
			model.setRequiredFertiliserActive(Str_fertiliserId, true);
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
		else if(action.equals("60_AddUserToModel")){
			String modelId 			= model.getId();
			String user				= request.getParameter("user");
			String rechte			= request.getParameter("rights");
			model.saveRightModel(modelId, user, rechte);
 			target = "03_showModel.jsp";
 			dispacher = request.getRequestDispatcher(target);
 			dispacher.forward(request, response);
 		}
		else if(action.equals("30_DeleteUserModel")){
			String modelId 			= model.getId();
			String user				= request.getParameter("user");		
			model.deleteUserOfModel(modelId, user);
			target = "03_showModel.jsp";
			dispacher = request.getRequestDispatcher(target);
			dispacher.forward(request, response);
		}
